...
class DKVMN_Options(nn.Module):
    def __init__(
        ...
        # Change output to predict 4 options instead of binary
        self.predict_linear = nn.Linear(self.final_fc_dim, 4, bias=True)
        ...
        # Change to encode 4 options (A,B,C,D) instead of binary (correct/incorrect)
        self.qa_embed = nn.Embedding(
            4 * self.n_question + 1, self.value_embedding_dim, padding_idx=0
        )
    ...
    def predict(self, q, s, pid=None):
        assert pid is None, "DKVMN_Options does not support pid input"
        q = q.masked_fill(q < 0, 0)
        s = s.masked_fill(s < 0, 0)
        # Encode question + chosen option (1-4) instead of question + correctness
        qa = q + (s - 1) * self.n_question
        q_embed_data = self.q_embed(q)
        qa_embed_data = self.qa_embed(qa)
        h = self(q_embed_data, qa_embed_data)
        # Output probabilities for 4 options
        return self.predict_linear(h), h
    ...
    def get_loss(self, q, s, pid=None):
        ...
        return F.cross_entropy(masked_logits, masked_labels.long())